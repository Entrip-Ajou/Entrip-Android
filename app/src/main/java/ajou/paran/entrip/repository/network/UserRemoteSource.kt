...
constructor(
    ...
    suspend fun saveUser(user: UserTemp): BaseResponse<UserRequest> = userApi.saveUser(user)
    ...
    suspend fun updateUserToken(user_id: String, token: String): BaseResult<Unit, Failure> {
        try {
            ...
            return if (res.status == 200) return BaseResult.Success(Unit)
                ...
                return BaseResult.Error(Failure(res.status, res.message))
            }
    ...
    suspend fun findById(user_id: String): BaseResponse<UserResponse> = userApi.findById(user_id)
