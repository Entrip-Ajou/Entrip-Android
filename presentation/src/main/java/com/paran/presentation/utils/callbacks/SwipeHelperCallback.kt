...
class SwipeHelperCallback(
    ...
    private fun getView(viewHolder: RecyclerView.ViewHolder): View = viewHolder.itemView.findViewById(R.id.cl_plan)

    private fun getDeleteView(viewHolder: RecyclerView.ViewHolder): View = viewHolder.itemView.findViewById(R.id.ll_item_delete)
    ...
    private fun clampViewPositionHorizontal(
        ...
        val max = 0f
        val newX = when (isClamped) {
            true -> when (isCurrentlyActive) {
                true -> when (dX < 0) {
                    true -> dX/3 - clamp
                    false -> dX - clamp
                }
